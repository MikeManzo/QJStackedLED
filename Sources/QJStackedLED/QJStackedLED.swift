...
/*
    ...
    public typealias NSColor = NSColor
...
open class QJStackedLED: QJView {
    ...
    @IBInspectable open var outerBorderColor: NSColor = NSColor.gray {
        didSet { updateControl() }
    }
    
    /// Inside border color
    @IBInspectable open var innerBorderColor: NSColor = NSColor.black {
        didSet { updateControl() }
    }
    
    /// The rendered segment color before reaching the warning threshold
    @IBInspectable open var normalColor: NSColor = NSColor.green {
        didSet { updateControl() }
    }
    
    /// The rendered segment color after reaching the warning threshold
    @IBInspectable open var warningColor: NSColor = NSColor.yellow {
        didSet { updateControl() }
    }
    
    /// The rendered segment color after reaching the danger threshold
    @IBInspectable open var dangerColor: NSColor = NSColor.red {
        didSet { updateControl() }
    }
    ...
    override open func draw(_ rect: CGRect) {
        ...
        for iX in 0..<numBars {
            ...
            var clrFill: NSColor = normalColor
    ...
    fileprivate func drawBar(_ a_ctx: CGContext, withRect a_rect: CGRect, andColor a_clr: NSColor, lit a_fLit: Bool) {
        // Is the bar lit?
        if a_fLit {
            ...
            if litEffect {
                ...
                // Set up color components from passed NSColor object
...
/// A view for showing a single number on an LED display
@IBDesignable
open class QJStackedLED: NSView {
    let controlDescription = "Stacked LED Control"
